...
def _data_prep(data_str):
    data_list = [data.strip() for data in data_str.split()]
    data_result = []
    for data in data_list:
        raw_date, raw_price, *other = data.split(',')
        raw_date, *other = raw_date.split('-')
        date = datetime.strptime(raw_date,'%Y')
        price = float(raw_price)
        data_result.append((date.year,price))
    return data_result

if __name__=="__main__":
    data = _data_prep(gold_prices)
    df = pd.DataFrame(data, columns=['Year','price'])
    df['diff'] = df.price.diff()
    max_increase = (df['diff'].max())
    max_decrease = (df['diff'].min())
    print(df.loc[df['diff'] == max_increase, 'Year'].values[0])
    print(df.loc[df['diff'] == max_decrease, 'Year'].values[0])