...
def operas_both_at_premiere(guest, composer):
    ...
    """
    if not composers.get(guest):
        raise ValueError("No such guest in the DB")
    elif not composers.get(composer):
        raise ValueError("No such composer in the DB")
    composer_born_date = _get_date(composers[composer].born)
    composer_die_date = _get_date(composers[composer].died)
    guest_born_date = _get_date(composers[guest].born)
    guest_die_date = _get_date(composers[guest].died)
    list_operas = []
    for opera in operas:
        opera_date = _get_date(opera.date)
        if opera.author == composer and opera_date < guest_die_date and opera_date > guest_born_date and opera_date < composer_die_date and opera_date > composer_born_date:
            list_operas.append(opera.play)
    return list_operas

if __name__=="__main__":
    print(len(operas_both_at_premiere('verdi','wagner')))